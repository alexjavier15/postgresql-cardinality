...
static char * getVarName(Index index) {
	...
	return (curr_rte->rte_table[index])->val.str;
...
void explicitNode(void *rte_ref, const void *obj, const void *str) {
	curr_rte = (RteReferences *) rte_ref;
	_outSimNode(S_NULL, (StringInfoData *) str, obj);

}

void nodeSimToString(void *rte_ref, const void *obj, const void *str) {
	curr_rte = (RteReferences *) rte_ref;
	_outSimNode(S_NULL, (StringInfoData *) str, obj);
...
char * nodeSimToString_(const void *obj) {
	...
	initStringInfo(&str);
	str.reduced = true;
	curr_rte = get_cur_rte_reference();
...
void _outSimVar(StringInfo str, const Var *node) {
	appendStringInfo(str, "%s", getVarName(node->varnoold));
	appendStringInfoSpaces(str, 1);
...
void _outSimRestrictInfo(StringInfo str, const RestrictInfo *node) {
	/* NB: this isn't a complete set of fields */
	_outSimNode(S_NULL, str, node->clause);
}

void _outSimNode(ArgType type, StringInfo str, const void *obj) {
