...
static bool choose_hashed_grouping(PlannerInfo *root, double tuple_fraction, double limit_tuples, double path_rows,
		int path_width, Path *cheapest_path, Path *sorted_path, double dNumGroups, AggClauseCosts *agg_costs);
static bool choose_hashed_distinct(PlannerInfo *root, double tuple_fraction, double limit_tuples, double path_rows,
		int path_width, Cost cheapest_startup_cost, Cost cheapest_total_cost, Cost sorted_startup_cost,
		Cost sorted_total_cost, List *sorted_pathkeys, double dNumDistinctRows);
...
subquery_planner(PlannerGlobal *glob, Query *parse, PlannerInfo *parent_root, bool hasRecursion, double tuple_fraction,
	...
	/* Also need to preprocess expressions within RTEs */foreach(l, parse->rtable) {
		RangeTblEntry *rte = (RangeTblEntry *) lfirst(l);
		int kind;
	...
	if (parse->resultRelation && rt_fetch(parse->resultRelation, parse->rtable)->inh)
		...
		if (parse->commandType != CMD_SELECT) {
			...
			plan = (Plan *) make_modifytable(root, parse->commandType, parse->canSetTag,
					list_make1_int(parse->resultRelation), list_make1(plan), withCheckOptionLists, returningLists,
...
inheritance_planner(PlannerInfo *root) {
	...
	if (subplans == NIL) {
		...
		return (Plan *) make_result(root, tlist, (Node *) list_make1(makeBoolConst(false,
						false)), NULL);
	}
	...
	return (Plan *) make_modifytable(root, parse->commandType, parse->canSetTag, resultRelations, subplans,
			withCheckOptionLists, returningLists, rowMarks, SS_assign_special_param(root));
}
...
grouping_planner(PlannerInfo *root, double tuple_fraction) {
	...
	if (parse->setOperations) {
		...
		tlist = postprocess_setop_tlist(copyObject(result_plan->targetlist), tlist);
		...
		if (parse->rowMarks)
			ereport(ERROR,
					...
					errmsg("%s is not allowed with UNION/INTERSECT/EXCEPT", LCS_asString(((RowMarkClause *) linitial(parse->rowMarks))->strength))));
		...
		if (parse->hasWindowFuncs) {
			wflists = find_window_functions((Node *) tlist, list_length(parse->windowClause));
		 ...
		 */
		if (parse->groupClause || parse->distinctClause || parse->hasAggs || parse->hasWindowFuncs
				|| root->hasHavingQual)
		...
		sorted_path = get_cheapest_fractional_path_for_pathkeys(final_rel->pathlist, root->query_pathkeys, NULL,
				tuple_fraction);
		...
		if (sorted_path) {
			...
			if (root->query_pathkeys == NIL || pathkeys_contained_in(root->query_pathkeys, cheapest_path->pathkeys)) {
				/* No sort needed for cheapest path */
				sort_path.startup_cost = cheapest_path->startup_cost;
				sort_path.total_cost = cheapest_path->total_cost;
			} else {
				/* Figure cost for sorting */
				cost_sort(&sort_path, root, root->query_pathkeys, cheapest_path->total_cost, path_rows, path_width, 0.0,
						work_mem, root->limit_tuples);
			}

			if (compare_fractional_path_costs(sorted_path, &sort_path, tuple_fraction) > 0) {
		...
		if (parse->groupClause) {
			 ...
			 */
			use_hashed_grouping = choose_hashed_grouping(root, tuple_fraction, limit_tuples, path_rows, path_width,
					cheapest_path, sorted_path, dNumGroups, &agg_costs);
			 ...
			 */
			use_hashed_distinct = choose_hashed_distinct(root, tuple_fraction, limit_tuples, path_rows, path_width,
					cheapest_path->startup_cost, cheapest_path->total_cost, sorted_path->startup_cost,
					sorted_path->total_cost, sorted_path->pathkeys, dNumGroups);
		...
		if (result_plan != NULL) {
			...
			if (use_hashed_grouping) {
				...
				result_plan = (Plan *) make_agg(root, tlist, (List *) parse->havingQual, AGG_HASHED, &agg_costs,
						numGroupCols, groupColIdx, extract_grouping_ops(parse->groupClause), numGroups, result_plan);
				...
				result_plan = (Plan *) make_agg(root, tlist, (List *) parse->havingQual, aggstrategy, &agg_costs,
						numGroupCols, groupColIdx, extract_grouping_ops(parse->groupClause), numGroups, result_plan);
			} else if (parse->groupClause) {
				/*
				...
				result_plan = (Plan *) make_group(root, tlist, (List *) parse->havingQual, numGroupCols, groupColIdx,
						extract_grouping_ops(parse->groupClause), dNumGroups, result_plan);
		...
		if (activeWindows) {
			 ...
			 */
			window_tlist = make_windowInputTargetList(root, tlist, activeWindows);
			...
			foreach(l, activeWindows) {
				...
				window_pathkeys = make_pathkeys_for_window(root, wc, tlist);
				...
				if (window_pathkeys) {
					...
					sort_plan = make_sort_from_pathkeys(root, result_plan, window_pathkeys, -1.0);
				...
				result_plan = (Plan *) make_windowagg(root, (List *) copyObject(window_tlist),
						wflists->windowFuncs[wc->winref], wc->winref, partNumCols, partColIdx, partOperators,
						ordNumCols, ordColIdx, ordOperators, wc->frameOptions, wc->startOffset, wc->endOffset,
						result_plan);
			}
		}
	...
	if (parse->distinctClause) {
		...
		if (!tested_hashed_distinct) {
			 ...
			 * "sorted".
			 */
			use_hashed_distinct = choose_hashed_distinct(root, tuple_fraction, limit_tuples, result_plan->plan_rows,
					result_plan->plan_width, result_plan->startup_cost, result_plan->total_cost,
					result_plan->startup_cost, result_plan->total_cost, current_pathkeys, dNumDistinctRows);
		}

		if (use_hashed_distinct) {
			/* Hashed aggregate plan --- no sort needed */
			result_plan = (Plan *) make_agg(root, result_plan->targetlist, NIL, AGG_HASHED, NULL,
					list_length(parse->distinctClause),
					extract_grouping_cols(parse->distinctClause, result_plan->targetlist),
					extract_grouping_ops(parse->distinctClause), numDistinctRows, result_plan);
			/* Hashed aggregation produces randomly-ordered results */
			current_pathkeys = NIL;
		} else {
			/*
			...
			if (!pathkeys_contained_in(needed_pathkeys, current_pathkeys)) {
				if (list_length(root->distinct_pathkeys) >= list_length(root->sort_pathkeys))
					...
					Assert(pathkeys_contained_in(root->distinct_pathkeys, current_pathkeys));
				}
	 ...
	 */
	if (parse->rowMarks) {
		result_plan = (Plan *) make_lockrows(result_plan, root->rowMarks, SS_assign_special_param(root));
	...
	if (limit_needed(parse)) {
		result_plan = (Plan *) make_limit(result_plan, parse->limitOffset, parse->limitCount, offset_est, count_est);
	}
...
void add_tlist_costs_to_plan(PlannerInfo *root, Plan *plan, List *tlist) {
	...
	plan->total_cost += tlist_cost.startup + tlist_cost.per_tuple * plan->plan_rows;
	...
	if (tlist_rows > 1) {
		 ...
		 */
		plan->total_cost += plan->plan_rows * (tlist_rows - 1) * cpu_tuple_cost / 2;
...
get_base_rel_indexes(Node *jtnode) {
	...
	if (IsA(jtnode, RangeTblRef)) {
		...
		result = NULL;
		foreach(l, f->fromlist)
			result = bms_join(result, get_base_rel_indexes(lfirst(l)));
	} else if (IsA(jtnode, JoinExpr)) {
		JoinExpr *j = (JoinExpr *) jtnode;
...
static void preprocess_rowmarks(PlannerInfo *root) {
	...
	if (parse->rowMarks) {
		 ...
		 */
		CheckSelectLocking(parse, ((RowMarkClause *) linitial(parse->rowMarks))->strength);
	} else {
		/*
...
static bool choose_hashed_grouping(PlannerInfo *root, double tuple_fraction, double limit_tuples, double path_rows,
	 ...
	 */
	can_hash = (agg_costs->numOrderedAggs == 0 && grouping_is_hashable(parse->groupClause));
	...
	if (!(can_hash && can_sort)) {
		if (can_hash)
			...
			ereport(ERROR,
					(errcode(ERRCODE_FEATURE_NOT_SUPPORTED), errmsg("could not implement GROUP BY"), errdetail("Some of the datatypes only support hashing, while others only support sorting.")));
	}
	 ...
	 */
	cost_agg(&hashed_p, root, AGG_HASHED, agg_costs, numGroupCols, dNumGroups, cheapest_path->startup_cost,
			cheapest_path->total_cost, path_rows);
	/* Result of hashed agg is always unsorted */
	if (target_pathkeys)
		cost_sort(&hashed_p, root, target_pathkeys, hashed_p.total_cost, dNumGroups, path_width, 0.0, work_mem,
				limit_tuples);
	...
	if (!pathkeys_contained_in(root->group_pathkeys, current_pathkeys)) {
		cost_sort(&sorted_p, root, root->group_pathkeys, sorted_p.total_cost, path_rows, path_width, 0.0, work_mem,
				-1.0);
		current_pathkeys = root->group_pathkeys;
	}

	if (parse->hasAggs)
		cost_agg(&sorted_p, root, AGG_SORTED, agg_costs, numGroupCols, dNumGroups, sorted_p.startup_cost,
				sorted_p.total_cost, path_rows);
	else
		cost_group(&sorted_p, root, numGroupCols, dNumGroups, sorted_p.startup_cost, sorted_p.total_cost, path_rows);
	/* The Agg or Group node will preserve ordering */
	if (target_pathkeys && !pathkeys_contained_in(target_pathkeys, current_pathkeys))
		cost_sort(&sorted_p, root, target_pathkeys, sorted_p.total_cost, dNumGroups, path_width, 0.0, work_mem,
				limit_tuples);
...
static bool choose_hashed_distinct(PlannerInfo *root, double tuple_fraction, double limit_tuples, double path_rows,
	...
	if (!(can_hash && can_sort)) {
		if (can_hash)
			...
			ereport(ERROR,
					(errcode(ERRCODE_FEATURE_NOT_SUPPORTED), errmsg("could not implement DISTINCT"), errdetail("Some of the datatypes only support hashing, while others only support sorting.")));
	}
	 ...
	 */
	if (parse->sortClause)
		cost_sort(&hashed_p, root, root->sort_pathkeys, hashed_p.total_cost, dNumDistinctRows, path_width, 0.0,
	...
	if (!pathkeys_contained_in(needed_pathkeys, current_pathkeys)) {
		if (list_length(root->distinct_pathkeys) >= list_length(root->sort_pathkeys))
			...
			current_pathkeys = root->sort_pathkeys;
		cost_sort(&sorted_p, root, current_pathkeys, sorted_p.total_cost, path_rows, path_width, 0.0, work_mem, -1.0);
	}
	...
	if (parse->sortClause && !pathkeys_contained_in(root->sort_pathkeys, current_pathkeys))
		cost_sort(&sorted_p, root, root->sort_pathkeys, sorted_p.total_cost, dNumDistinctRows, path_width, 0.0,
				work_mem, limit_tuples);
 ...
 */
static List *
make_subplanTargetList(PlannerInfo *root, List *tlist, AttrNumber **groupColIdx, bool *need_tlist_eval) {
	Query *parse = root->parse;
	List *sub_tlist;
	List *non_group_cols;
	List *non_group_vars;
	...
	*groupColIdx = NULL;
	 ...
	 */
	if (!parse->hasAggs && !parse->groupClause && !root->hasHavingQual && !parse->hasWindowFuncs) {
		*need_tlist_eval = true;
		return tlist;
	}
		...
		*groupColIdx = grpColIdx;
		...
		foreach(tl, tlist) {
			...
			if (colno >= 0) {
				...
				newtle = makeTargetEntry(tle->expr, list_length(sub_tlist) + 1, NULL, false);
				sub_tlist = lappend(sub_tlist, newtle);

				Assert(grpColIdx[colno] == 0);
				/* no dups expected */
 ...
 */
static List *
make_windowInputTargetList(PlannerInfo *root, List *tlist, List *activeWindows) {
	Query *parse = root->parse;
	Bitmapset *sgrefs;
	List *new_tlist;
	...
	/* Add in sortgroupref numbers of GROUP BY clauses, too */foreach(lc, parse->groupClause) {
...
make_pathkeys_for_window(PlannerInfo *root, WindowClause *wc, List *tlist) {
	...
	if (!grouping_is_sortable(wc->partitionClause))
		ereport(ERROR,
				(errcode(ERRCODE_FEATURE_NOT_SUPPORTED), errmsg("could not implement window PARTITION BY"), errdetail("Window partitioning columns must be of sortable datatypes.")));
	if (!grouping_is_sortable(wc->orderClause))
		ereport(ERROR,
				(errcode(ERRCODE_FEATURE_NOT_SUPPORTED), errmsg("could not implement window ORDER BY"), errdetail("Window ordering columns must be of sortable datatypes.")));
	...
	if (numSortCols == numPart + numOrder) {
		...
		*ordOperators = (Oid *) palloc(numOrder * sizeof(Oid));
		...
		foreach(lc, wc->partitionClause) {
			...
			new_pathkeys = make_pathkeys_for_sortclauses(root, sortclauses, tlist);
...
bool plan_cluster_use_sort(Oid tableOid, Oid indexOid) {
	...
	cost_sort(&seqScanAndSortPath, root, NIL, seqScanPath->total_cost, rel->tuples, rel->width, comparisonCost,
			maintenance_work_mem, -1.0);
