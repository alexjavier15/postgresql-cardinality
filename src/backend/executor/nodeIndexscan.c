...
IndexNext(IndexScanState *node) {
	...
	while ((tuple = index_getnext(scandesc, direction)) != NULL) {
		...
		/*if (!bms_is_member(pointer, node->ss.ps.instrument->bmp)) {

			bms_add_member(node->ss.ps.instrument->bmp, (pointer - node->ss.ps.instrument->base + 1));
			node->ss.ps.instrument->tuple_dist++;
		}*/
		/*
 ...
 */
void ExecIndexBuildScanKeys(PlanState *planstate, Relation index, List *quals, bool isorderby, ScanKey *scanKeys,
	...
	foreach(qual_cell, quals) {
		...
		if (IsA(clause, OpExpr)) {
			...
			while (opnos_cell != NULL) {
				 ...
				 */
				ScanKeyEntryInitialize(this_sub_key, flags, varattno, /* attribute number */
				op_strategy, /* op's strategy */
				op_righttype, /* strategy subtype */
